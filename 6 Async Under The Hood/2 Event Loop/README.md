...
Итак, у нас есть функция `_connect`. Она принимает `sock`, `host` и `port`, к которым нужно подключиться.
...
class Fetcher:
    def fetch(self):
        ...
        for f in connect(self.sock, 'abcd.com', 80):
            yield f
```

Задача - отрефакторить код так, чтобы могли использовать функцию `connect` в любой момент. Мы находимся в нашей функции `fetch`. Мы создали сокет и итерируемся по всем `Future`, которые за-yield'ит функцию `connect()`.

Например, `connect()` подключается к сокету и начинает в него писать, то есть `connect` делает один `yield` для соединений и один `yield` для записи. Мы выступаем в роли какого-то посредника - `connect()` отдаем нам сколько-то `Future`, а мы отдаем их дальше.

Это выглядит не очень удобно - любой генератор, у которого есть хотя бы два `yield`, обязывает нас использовать цикл как в примере. А если в генераторе тоже используется цикл, то тем более.
...
class Fetcher:
    def fetch(self):
        self.sock = socket.socket()
        self.sock.setblocking(False)
        yield from connect(self.sock, 'abcd.com', 80)
```

Первый и второй примеры работают почти идентично. Единственное - но важное - отличие видно при завершении работы генератора. Если мы будем идти целиком по генератору, однажды генератор закончится и кинет ошибку `StopIteration`. Просто `yield` выкинет эту ошибку дальше, а `yield from` перехватит и вернет последнее возвращенное значение (это значение хранится в ошибке). То есть, когда в конце генератора вызываем `return `, выбрасывает исключение `StopIteration` с `StopIteration.value = 1`.

Поэтому, чтобы примеры работали идентично, первый пример надо переписать:
...
## Пишем read, send и read_all

Методы чтения из сокета чанка (read), записи в сокет (send) и чтение всех данных из сокета (read_all) очень похожи на метод `_connect`, который мы рассматривали раннее.

Смотри `2_socket_methods.py`.

### read

Посмотрим на функцию `read()` более внимательно - в последних двух строках происходит небольшая магия.

Нам необходимо убедиться в том, что сокет доступен на чтение `EVENT_READ`, и только в этот момент мы пытаемся вычитать что-то из сокет. Мы делаем `sock.recv(4096)`, то есть, 4096 байт. Не факт, что мы столько прочитаем - главное, что прочитаем хотя бы часть
...
Зарегистрировали callback на `on_readable` и передали событие `EVENT_READ`: попросили ОС вызвать нам `on_readable`, когда сокет станет доступным на чтение. Сделали `yield` для этой `Future` и сказали, что получим из нее чанк.

В `Task` в методе `send()` передается `future_result` - результат предыдущей выполненной `Future`.

Когда `Future` завершится, продолжится исполнение с последнего `yield`, и все, что передается в `send`, передается в качестве возвращаемого значения из `yield`. Так устроены генераторы в Python. То есть, мы получили из этой `Future` результат и послали его в тот же генератор. Генератор поставил результат на место последнего `yield`, то есть сюда: `chunk = yield f`.

Соответственно, при следующем движении генератора `yield f` заменится на некоторый набор байт, который присвоится в `chunk`. А затем мы просто делаем `return chunk`.

Мы могли бы просто вернуть в конце `return f.result`, но это не очень читаем, правда?

### read_all

Код получается более привычный: просто дописан синтаксический сахар `yield from`.

Код очень простой и похож на линейный: мы делаем `read` и получаем `chunk`: `chunk = yield from read(sock)`. До тех пор, пока есть чанки, мы все это суммируем и делаем `read` еще раз. Обычный бесконечный цикл до тех пор, пока мы не вычитаем все содержимое из сокета и не вернем результат: `Task.step` -> `fetch` -> `read_all` -> `read` и наоборот.

У нас есть `Task.step`, который делает `self.coro.send(result)`, в котором записан `None`. Когда дергается `fetch`, все это идет по цепочке, опускаясь до первого `yield`, в котором произойдет в функции `read`. Далее все это дело поднимается обратно до `Task.step`, который передаст callback: "Когда `Future` завершится, вызови меня (`step`) еще раз".

Далее все засыпает на `select`. Это самый важный момент. Наше приложение спит и ждет события на сокетах. Как только оно просыпается, вызывается какая-то внутренняя функция, в данном случае - callback-функция `_read`.

`Future` завершилась благодаря вызову `set_result`, вызывается callback, чтобы еще раз вызвать `step`. Снова делается `send` и продолжает исполнение по цепочке до ближайшего `yield`. Сделался `yield`, и она опять пошла наверх и ждет. То есть, как только сделался `yield`, мы ждем: приложение условно останавливаетсяя.
